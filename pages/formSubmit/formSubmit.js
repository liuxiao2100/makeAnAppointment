...
Page({
  ...
  // 哪个单位？
  bindWhichCompany: function (e) {
    this.setData({company: e.detail.value})
  },
  // 是否预约B超
  bindIsBChao: function (e) {
    console.log('Bchao'+e.detail.value)
    this.setData({isBChao: e.detail.value})
  },
  // 输入日期
  ...
  btnClick: function (e) {
    // test
    // var that = this;
    // console.log('时间 -- '+that.data.uDate);
    // var d = new Date();
    // console.log('星期 -- '+d.getDay());
    // console.log('字符串转Date -- '+ Date.parse(that.data.uDate))
    // var p = Date.parse(that.data.uDate)
    // var b =  new Date()
    // b.setTime(p)
    // console.log('集合起来'+ b.getDay())
    ...
    var d = new Date();
    var b = Date.parse(that.data.uDate)
    d.setTime(b);
    ...
    if (that.data.uName.length == 0 || that.data.uID.length == 0 || that.data.uPhone.length == 0 || that.data.isMale.length == 0) {
      ...
      util.showModel('提交预约失败', that.data.uDate+'为周日，请选择其他日期')
